...
import { KeyboardAvoidingView, TouchableWithoutFeedback, ScrollView, Dimensions } from 'react-native';
 ...
 const editorStyles =  {
	...
	return (
		<SafeAreaView style={{ flex: 1, backgroundColor: 'white' }}>
			...
			<KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.container} keyboardVerticalOffset={110}>
				{/*<TouchableWithoutFeedback onPress={Keyboard.dismiss}>
                    <>*/}
					<ScrollView contentContainerStyle={{paddingTop: 15, paddingBottom: 100}}>
                    	{comments.map(({id, data}) => <CommentComp userId={uid} postId={postId} key={id} id={id} data={data} uid={data?.uid} />)}
                    </ScrollView>
					{/*<View style={styles.footer}>*/}
					<View style={{backgroundColor: '#FFFFFF', position: 'absolute', bottom: 0, flexDirection: 'row', height: 90, width: windowWidth, alignItems: 'center', borderTopWidth: 0.5, borderTopColor: '#C7C6CE'}}>
						...
						<TouchableOpacity onPress={sendMessage} style={{backgroundColor: '#D2FFF4', width: 50, height: 50, borderRadius: 100, alignItems: 'center', justifyContent: 'center'}}>
							<Ionicons name={'ios-send-outline'} size={30} color={'#2EDDB8'}/>
						</TouchableOpacity>
					</View>
					<TextInput style={{position: 'absolute', bottom: 0, marginLeft: 20, fontSize: 18, marginBottom: 51}} placeholder={'Type a message...'} onChangeText={setInput} value={input}/>
