...
const UserFollowers = ({ navigation, route }) => {
    ...
    navigation.setOptions({
      ...
      headerRight: () => (
        <Menu
          ref={setMenuRef}
          button={
            <TouchableOpacity style={{ margin: 10 }} onPress={showMenu}>
  ...
  const onAdd = () => {
    if (value) {
      let u = users.filter(
        (user) => user.displayName === selectedItem.displayName
      )[0];
  ...
  return (
    <ScrollView style={{ backgroundColor: "white" }}>
      ...
      {(!filteNewToOld || !filterOldToNew ? list : filteredList)?.map(
        (l, i) => (
