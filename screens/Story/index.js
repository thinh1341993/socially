...
export default function Story({ navigation, route }) {
  ...
  useEffect(() => {
    if (deletePost) {
        ...
        [
          {
            ...
            onPress: () => 
            {
              console.log('Deleting post ID: ' + content[current].id);
              db.collection('users').doc(user?.uid).collection('stories').doc(content[current].id).delete().then(() => 
              {
                console.log(content[current].id + ' has been deleted');
              });
            },
          },
          {
      ...
      //navigation.navigate('DeletePost', { postID: content[current].id, userID: user?.uid });
      sound.unloadAsync();
    }
  }, [deletePost]);
            ...
            <View
              ...
              <View
                ...
                <View style={{ flexDirection: "column", alignItems: "center" }}>
                  ...
                  <TouchableOpacity onPress={() => setDeletePost(true)}>
                    <View
                      style={{
                        alignItems: 'center',
                        justifyContent: 'center',
